...
weekday_weekend_pvalue = kstest(weekday_dur, weekend_dur).pvalue
...
borough_groups = []
for borough in nyc311_NYPD['Borough'].unique():
    borough_group = nyc311_NYPD[nyc311_NYPD['Borough'] == borough]['Duration (hours)']
    borough_groups.append(borough_group)

# Perform the borough comparisons
borough_pvalues = []
for group1, group2 in combinations(borough_groups, 2):
    pvalue = kstest(group1, group2).pvalue
    borough_pvalues.append(pvalue)

print('Borough p-values:', borough_pvalues)
```
